...
clustalo_exe = r"C:/Users/lourd/OneDrive/Documentos/00_bioinfo/bioinfo-tp/Lou_campeona/clustal-omega-1.2.2-win64/clustalo.exe"  # <--- NUEVO
...
output_clustalo = r"C:/Users/lourd/OneDrive/Documentos/00_bioinfo/bioinfo-tp/Lou_campeona/output_clustalo"  # <--- NUEVO
os.makedirs(output_muscle, exist_ok=True)
os.makedirs(output_mafft, exist_ok=True)
os.makedirs(output_clustalo, exist_ok=True)  # <--- NUEVO
...
tmp_prot_clustalo = os.path.join(output_clustalo, "all_proteins.fasta")  # <--- NUEVO
tmp_dna_clustalo = os.path.join(output_clustalo, "all_dna.fasta")        # <--- NUEVO
...
# 1. Clasifica y guarda las secuencias para MUSCLE, MAFFT y CLUSTALO
with open(tmp_prot, "w") as prot_out, open(tmp_dna, "w") as dna_out, \
     ...
     open(tmp_prot_clustalo, "w") as prot_out_clustalo, open(tmp_dna_clustalo, "w") as dna_out_clustalo:
        ...
        if fname.endswith(".fasta") or fname.endswith(".fa"):
            for record in SeqIO.parse(os.path.join(fastas_folder, fname), "fasta"):
                ...
                if is_dna(record.seq):
                    ...
                    SeqIO.write(record, dna_out_clustalo, "fasta")
                else:
                    SeqIO.write(record, prot_out, "fasta")
                    SeqIO.write(record, prot_out_mafft, "fasta")
                    SeqIO.write(record, prot_out_clustalo, "fasta")
...
def run_clustalo(input_fasta, output_fasta):
    if os.path.exists(input_fasta) and os.path.getsize(input_fasta) > 0:
        cmd = [clustalo_exe, "-i", input_fasta, "-o", output_fasta, "--force", "--outfmt=fasta"]
        print("Ejecutando:", " ".join(cmd))
        result = subprocess.run(cmd, capture_output=True, text=True)
        if result.returncode != 0:
            print(f"Error ejecutando CLUSTALO para {input_fasta}:")
            print(result.stderr)
            return None
        return output_fasta
    else:
        print(f"No hay secuencias en {input_fasta}, no se ejecuta CLUSTALO.")
        return None
...
prot_aln_clustalo = run_clustalo(tmp_prot_clustalo, out_prot_clustalo)
dna_aln_clustalo = run_clustalo(tmp_dna_clustalo, out_dna_clustalo)
